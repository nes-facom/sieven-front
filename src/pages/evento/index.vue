      ...
      <v-col class="mr-8">
        <v-row id="containerBase">
          <div id="imageConteiner">
            <v-img :src="evento.imagem" id="cardBase" class="mx-auto"> </v-img>
            <v-btn
              v-if="this.$store.getters.isAdmin"
              class="excluir-evento-button"
              color="white"
              outlined
            ...
            <v-dialog v-model="excluirDialogConfirmacao" max-width="750">
              <v-card class="excluirDialog">
                <v-card-title
                  ...
                  style="color: #097fa8; text-align: center; margin-left: 37%"
                >
                  Atenção!
                </v-card-title>
                <v-card-text>
                  <p
                    ...
                    "
                  >
                    Tem certeza que deseja remover esse evento? Todas as suas
                    informações e atividades serão excluídas em definitivo.
                  </p>
                </v-card-text>
                <v-card-actions>
                  <v-btn
                    color="#097FA8"
                    text
                    @click="cancelarEventoExclusao"
                    style="margin-left: 72%"
                    >Cancelar</v-btn
                  >
                  <v-btn color="red" text @click="confirmarEventoExclusao"
                    >Remover</v-btn
                  >
                </v-card-actions>
              </v-card>
            ...
            >
              ...
              <editar-evento-dialog
                @fecharEditarEventoDialog="fecharEditarEventoDialog"
              >
              </editar-evento-dialog>
            </v-dialog>
          </div>
      ...
      >
        <template v-slot:activator="{ on, attrs }">
          <v-card
            ...
            outlined
          >
            <v-card-title class="py-2 justify-left" style="color: #097fa8">
              {{ atividade.nome }}
            </v-card-title>
            <v-divider class="mx-6"></v-divider>
            <v-card-subtitle
              class="py-2 justify-center text-justify"
              style="color: #8b8b8b"
            >
            ...
            <v-row class="mx-4">
              <div v-if="isAdmin()">
                <v-col class="pa-0">
                  <v-dialog v-model="atividade.editarDialog" width="700">
                    <template v-slot:activator="{ on, attrs }">
                      <v-btn
                        style="color: white"
                        depressed
                  ...
                  <v-dialog v-model="atividade.excluirDialog" width="512">
                    <template v-slot:activator="{ on, attrs }">
                      <v-btn
                        class="ml-2"
                        style="color: white"
    ...
    >
      <template v-slot:activator="{ on, attrs }">
        <v-btn
          ...
          v-on="on"
        >
          <v-icon class="mr-2" size="30"> mdi-plus </v-icon>
          Criar Atividade
        </v-btn>
      </template>
...
export default {
  ...
  methods: {
      ...
      apiEvento
        ...
        .then((response) => {
          ...
          this.evento = {
            ...
            imagem:
              eventoResponse.imagem ||
              "https://img2.gratispng.com/20180510/sxq/kisspng-boulder-theater-computer-icons-ticket-cinema-5af509b9cdcea8.635574511526008249843.jpg",
    ...
    confirmarEventoExclusao() {
      apiEvento
        .deletarEventos(
          middleware.recuperarToken("token").access_token,
          this.evento.id
        )
        .then((response) => {
